...
export default function BranchCustomerChatsPage() {
  ...
  // Debug auth
  useEffect(() => {
    console.log('🔐 Auth State:', { user, id: user?.id, role: user?.role, branchId: user?.branchId });
    if (user) {
      setIsLoading(false);
    }
  }, [user]);
      ...
      try {
        ...
        if (branchesData.length > 0) {
          if (user?.role === 'admin' && user?.branchId) {
            // Branch admin - select their branch
            setSelectedBranchId(user.branchId);
          } else {
            // Super admin - select first branch
  ...
  // Fetch all customer chats for SELECTED branch from customerMessages collection
  useEffect(() => {
    if (!selectedBranchId) return;
    
    const fetchAllChats = async () => {
      try {
        console.log(`🔍 Fetching customers for branch: ${selectedBranchId}`);
        
        // Read from customerMessages collection
        ...
        console.log(`✅ Found ${chatsData.length} customers for ${selectedBranchDetails?.name || selectedBranchId}`);
        setChats(chatsData);
      } catch (error) {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Message data for branch admin to customer - SAVES IN customerReplies collection
      const messageData: any = {
        content: messageContent,
        senderId: user.id,
        senderName: selectedBranchDetails.name,
        senderEmail: selectedBranchDetails.email || user.email || '',
  ...
  useEffect(() => {
    ...
    // Query customerMessages (incoming from customer)
    ...
    // Query customerReplies (outgoing from branch admin)
  ...
  const getBranchDisplayName = () => {
    if (selectedBranchDetails) {
      return selectedBranchDetails.name;
    }
    if (myBranchId && myBranchDetails) {
      return myBranchDetails.name;
    }
                ...
                <div className="p-2">
                  {!selectedBranchId ? (
                    ...
                    <div className="text-center py-8 px-4">
                      ...
                      <p className="text-sm text-gray-500">No customer chats yet</p>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center gap-4">
                    <div className="flex-1">
                      ...
                      {user?.role === 'admin' && user?.branchId ? (
                        /* Branch Admin - Show branch name directly */
                                    ...
                                    <div className="flex items-start gap-3">
                                      ...
                                      <div className="flex-1">
                                        ...
                                        <div className="flex items-center gap-2 mt-1">
                                          ...
                                          {myBranchId === branch.id && (
                                            <Badge className="bg-green-100 text-green-700 border-0 text-[8px] px-2">
                                              Your Branch
                                            </Badge>
                                          )}
                                        </div>
                                              ...
                                              <div className="relative group shrink-0">
                                                <Avatar className="w-8 h-8 ring-2 ring-white shadow-md">
                                                  <AvatarFallback className="bg-gradient-to-br from-blue-600 to-blue-800 text-white text-xs">
                                                    {selectedBranchDetails?.name?.charAt(0) || 'B'}
                                                  </AvatarFallback>
                                                </Avatar>
                                              </div>
                                            )}
