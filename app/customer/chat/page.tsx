...
function CustomerChatPage() {
  ...
  const currentUserName = user?.name || user?.email?.split('@')[0] || 'Customer';

  // ✅ REMOVED LOADING STATE - Directly check user
  useEffect(() => {
    console.log('🔐 Auth State - User object:', user);
    console.log('🔐 Auth State - User ID:', user?.id);
    console.log('🔐 Auth State - User Email:', user?.email);
    console.log('🔐 Auth State - User Name:', user?.name);
  }, [user]);
  ...
  // Fetch ALL chats for customer (from both collections)
  useEffect(() => {
    if (branches.length === 0 || !currentUserId) return;
    ...
    const fetchAllChats = async () => {
      try {
        ...
        for (const branch of branches) {
          // Get messages from customerMessages (customer sent)
          ...
          // Get replies from customerReplies (branch admin sent to this customer)
          const customerRepliesQuery = query(
            collection(db, 'customerReplies'),
            where('recipientCustomerId', '==', currentUserId),
            where('recipientBranchId', '==', branch.id)
          );
          
          const [customerSnapshot, repliesSnapshot] = await Promise.all([
            getDocs(customerMessagesQuery),
            getDocs(customerRepliesQuery)
          ]);
          ...
          // Client-side sorting for last message
          allMessages.sort((a, b) => b.timestamp - a.timestamp);
          
          const lastMessage = allMessages[0];
          
          // Calculate unread count for this branch (from branch replies only)
          let unreadCount = 0;
    ...
    fetchAllChats();
  }, [branches, currentUserId, selectedBranchId]);
  ...
  // ✅ Send message to branch (customerMessages collection)
  const handleSendMessage = async () => {
    ...
    if (!user.id) {
      ...
      console.error('❌ User object missing id - Full user:', user);
      return;
    }
    ...
    try {
      ...
      // ✅ Message saves in 'customerMessages' collection
      ...
      console.log('📤 Sending message to customerMessages collection:', messageData);

      const docRef = await addDoc(collection(db, 'customerMessages'), messageData);
  ...
  // ✅ Fetch messages from BOTH collections for selected branch
  useEffect(() => {
    ...
    console.log('🔍 Setting up message listener for branch:', selectedBranchId);

    // Listen to customerMessages (messages sent by customer)
    const customerMessagesQuery = query(
      collection(db, 'customerMessages'),
      where('senderId', '==', currentUserId),
    ...
    // Listen to customerReplies (replies from branch admin to this customer)
    const customerRepliesQuery = query(
      collection(db, 'customerReplies'),
      where('recipientCustomerId', '==', currentUserId),
      where('recipientBranchId', '==', selectedBranchId)
    ...
    const unsubscribeCustomerMessages = onSnapshot(customerMessagesQuery, (snapshot) => {
      console.log('📨 Customer messages snapshot received:', snapshot.docs.length);
      
      const customerMsgs = snapshot.docs.map(doc => {
        const data = doc.data();
        
        // ✅ Update status for customer's own messages (1 tick -> 2 ticks)
        if (data.senderRole === 'customer' && data.status === 'sent') {
          setTimeout(() => {
            markMessageAsDelivered(doc.id, 'customerMessages');
          }, 500);
        }
      ...
      console.error('❌ Customer messages snapshot error:', error);
    });

    const unsubscribeCustomerReplies = onSnapshot(customerRepliesQuery, (snapshot) => {
      console.log('📨 Customer replies snapshot received:', snapshot.docs.length);
      
      const replyMsgs = snapshot.docs.map(doc => {
        const data = doc.data();
        
        // ✅ Don't update status for branch replies - just display them
        // We only need to know the status for our own sent messages
      ...
      // Merge with existing customer messages
      setMessages(prev => {
      ...
      console.error('❌ Customer replies snapshot error:', error);
    });
  ...
  // ✅ REMOVED LOADING STATE - Direct render
  // Not logged in state
  if (!user) {
    return (
      <div className="h-screen flex items-center justify-center">
        <div className="text-center max-w-md px-6">
          ...
          <p className="text-gray-600 mb-6">Please log in to access customer support chat.</p>
          <Button 
                  ...
                  {filteredChats.length === 0 ? (
                    <div className="text-center py-8 px-4">
                      ...
                      <p className="text-xs text-gray-400 mt-1">
                        Select a branch below to start
                      </p>
                    </div>
                  ) : (
                              ...
                              <div className="flex-1">
                                ...
                                {chat.lastMessage ? (
                                  <p className="text-xs text-gray-400 truncate mt-0.5">
                                    {chat.lastMessage.senderRole === 'customer' ? 'You: ' : ''}
                      ...
                      <div className="flex items-center gap-4 px-4 py-2 bg-[#FA9DB7]/5 rounded-2xl border border-[#FA9DB7]/20 shrink-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-gray-600">
                            {displayBranchDetails.openingTime || '09:00'} - {displayBranchDetails.closingTime || '18:00'}
                          </span>
                        </div>
                      </div>
                    )}
                                ...
                                <div key={date}>
                                  ...
                                  <div className="space-y-3">
                                    {(dateMessages as Message[]).map((msg) => {
                                      ...
                                      const isFromBranch = msg.senderRole === 'branch_admin';
                                        ...
                                        <div key={msg.id} className={cn("flex items-end gap-2", isMe ? "justify-end" : "justify-start")}>
                                          ...
                                          <div className={cn("max-w-xs lg:max-w-md", isMe ? "order-2" : "order-1")}>
                                            <div className={cn(
                                              ...
                                              {/* ✅ Time & Status - Only show ticks for messages sent by customer */}
                                              <div className={cn(
                                                "flex items-center justify-end gap-1 mt-1 text-[10px]",
                                                isMe ? "text-gray-600" : "text-gray-400"
                                              )}>
                                                <span>{formatMessageTime(msg.timestamp)}</span>
                                                {/* For customer's own messages (sent to branch) - SHOW TICKS */}
                                                {isMe && (
                                                  <>
                                                    {msg.status === 'sent' && <Check className="w-3 h-3" />}
                                                    {msg.status === 'delivered' && <CheckCheck className="w-3 h-3" />}
                                                    {msg.status === 'seen' && <CheckCheck className="w-3 h-3 text-blue-600" />}
                                                  </>
                                                )}
                                                {/* For branch replies (received by customer) - NO TICKS, just time */}
                                                {/* {!isMe && isFromBranch} - Renders nothing extra, just time */}
                                              </div>
                  ...
                  <div className="flex-1 flex items-center justify-center bg-gradient-to-b from-gray-50 to-white">
                    <div className="text-center max-w-md px-6">
                      ...
                      <p className="text-gray-500 mb-8">
                        Select a branch from the dropdown above or choose from your recent chats to start messaging.
                      </p>
