...
export default function SuperAdminMembership() {
  ...
  // 🔥 Firebase se real-time data fetch
  useEffect(() => {
    // Fetch Memberships
    let unsubscribeMemberships: Unsubscribe | undefined;
    const fetchMemberships = async () => {
      try {
        const membershipsRef = collection(db, 'memberships');
        const q = query(membershipsRef, orderBy('createdAt', 'desc'));
        
        unsubscribeMemberships = onSnapshot(q, (snapshot) => {
    ...
    // Fetch Offers
    let unsubscribeOffers: Unsubscribe | undefined;
    ...
    // Fetch PromoCodes
    let unsubscribePromoCodes: Unsubscribe | undefined;
    const fetchPromoCodes = async () => {
      try {
        const promoCodesRef = collection(db, 'promoCodes');
        const q = query(promoCodesRef, orderBy('createdAt', 'desc'));
        
        unsubscribePromoCodes = onSnapshot(q, (snapshot) => {
    ...
    let unsubscribeCashback: Unsubscribe | undefined;
    const fetchCashbackPrograms = async () => {
      try {
        const cashbackRef = collection(db, 'cashbackPrograms');
        const q = query(cashbackRef, orderBy('createdAt', 'desc'));
        
        unsubscribeCashback = onSnapshot(q, (snapshot) => {
          ...
          setCashbackPrograms(cashbackData);
        }, (error) => {
          console.error("Error fetching cashback programs: ", error);
        });
      } catch (error) {
        console.error("Error in fetchCashbackPrograms: ", error);
      }
    };

    // Fetch Loyalty Points
    let unsubscribeLoyalty: Unsubscribe | undefined;
    const fetchLoyaltyPoints = async () => {
      try {
        const loyaltyRef = collection(db, 'loyaltyPoints');
        const q = query(loyaltyRef, orderBy('createdAt', 'desc'));
        
        unsubscribeLoyalty = onSnapshot(q, (snapshot) => {
    ...
    return () => {
      ...
      if (unsubscribeCashback) unsubscribeCashback();
      if (unsubscribeLoyalty) unsubscribeLoyalty();
    };
  }, []);
            ...
            <div className="flex items-center justify-between px-4 py-4 lg:px-8">
              <div className="flex items-center gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-600">Manage memberships, offers, promo codes, cashback & loyalty programs</p>
                </div>
              </div>
              <div className="flex items-center gap-4">
